...
capability (`AdapterSite`) outlives one named for its current implementation (`LoRALinear` is the
*type*, not the protocol). A backend named for its model (`nanodiff.py`, `llada.py`) means a new
base model is a new file. A name that describes today's implementation will be a lie next
quarter.
