"""What the status bar shows: GPUs, the base checkpoint, trained artifacts, the last run."""
...
def snapshot(cfg: Config) -> Snapshot:
    """The status bar's view of the repo, read from disk and nvidia-smi."""
    ...
    records = load_records(cfg.paths.runs)
    last = f"{records[-1].kind} {records[-1].started_at[5:16].replace('T', ' ')}" if records else ""
    return Snapshot(
        gpus=gpus(),
